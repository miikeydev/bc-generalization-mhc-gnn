...
def _generate_split(
    ...
    for _ in range(count):
        ...
        raw_bc = _compute_betweenness_centrality(graph, bc_approximation_k, graph_seed)
...
def _compute_betweenness_centrality(graph: nx.Graph, approximation_k: int | None, seed: int) -> np.ndarray:
    if approximation_k is not None:
        bc = nx.betweenness_centrality(graph, normalized=True, k=approximation_k, seed=seed)
    else:
