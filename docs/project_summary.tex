...
$+127\%$ Kendall gain.
The routing matrix analysis (Section~\ref{sec:matrices}) provides
...
Post-hoc analysis of the inter-stream residual matrices $H_\text{res}$
on three models across three regimes (synthetic ID, synthetic OOD,
Cora), 5 seeds.
...
\toprule
Models & \texttt{hc\_gcn\_l8}, \texttt{mhc\_gcn\_l8},
         \texttt{mhc\_lite\_gcn\_l8} \\
Regimes & synthetic ID, synthetic OOD, Cora \\
Seeds & 5 \\
Layers & all 8 \\
Statistics & identity distance, row entropy,
             row/col sum error, permutation proximity \\
\bottomrule
\end{tabular}
\end{table}

\subsection{HC-GCN: progressive, unconstrained routing freedom}

$H_\text{res}$ drifts progressively away from the identity
as depth increases (measured on synthetic OOD):

\begin{table}[h]
\centering
\begin{tabular}{C{2.5cm} R{3.5cm} R{3.5cm}}
\toprule
...
Row and column sum errors become large in deeper layers,
confirming that $H_\text{res}$ is not doubly stochastic:
the model is free to route probability mass unevenly across streams.
This aggressive, unconstrained remixing is consistent with
HC-GCN's superiority on the synthetic inductive benchmark,
and explains the routing collapse on CiteSeer
(Section~\ref{sec:citeseer-collapse}):
deep layers over-commit to bridge-node streams, distorting global rankings.

\subsection{mHC-GCN: strongly mixed but Sinkhorn-balanced}

The raw residual matrix is already far from the identity
before Sinkhorn projection (layer~7:
raw identity distance $\approx 0.816$):
the model \emph{wants} to mix streams aggressively.
After Sinkhorn projection:

\begin{itemize}[noitemsep]
  \item identity distance at layer 7 $\approx 0.403$ (synthetic OOD)
  \item row and column sum errors $\approx 0$
        (doubly-stochastic constraint enforced)
  \item the pattern is stable across synthetic ID, synthetic OOD,
        and Cora
\end{itemize}

The projection forces the mixing into a balanced transport plan,
preserving more global structure than HC.
This is consistent with mHC's stronger zero-shot transfer
and the absence of regression across all tested real graphs.

\subsection{mHC-lite-GCN: near-uniform, depth-invariant mixer}

With $n_\text{str}=2$, the routing behaves like a near-uniform
convex combination of identity and swap, almost constant across
layers and regimes:

\begin{itemize}[noitemsep]
  \item identity distance $\approx 0.707$ (constant across layers)
  \item row entropy $\approx 1.0$ (maximum)
  \item max permutation coefficient $\approx 0.50$
  \item permutation entropy $\approx 1.0$
\end{itemize}

The mechanism is only weakly input-adaptive, which is consistent
with its stable but moderate performance on large real graphs
such as PubMed.

\subsection{Mechanistic summary}
\label{sec:citeseer-collapse}

\begin{table}[h]
\centering
\caption{Routing characterisation from learned $H_\text{res}$ matrices}
\begin{tabular}{L{2.8cm} L{3.8cm} L{4.8cm}}
\toprule
Variant & Routing character & Consequence \\
\midrule
HC       & Free; grows more aggressive at depth
         & Strongest synthetic BC; collapses on sparse/skewed graphs \\
mHC      & Strongly mixed, Sinkhorn-balanced
         & Best zero-shot transfer; no regression on any graph \\
mHC-lite & Near-uniform, depth-invariant
         & Stable and competitive on large graphs (PubMed) \\
\bottomrule
\end{tabular}
\end{table}

The synthetic vs.\ real ranking inversion is not a metric artefact:
it has a clear mechanistic signature in the routing matrices.
HC and mHC learn structurally different transport plans,
and those differences map directly onto their respective
failure modes.
...
\begin{enumerate}
  ...
  \item \textbf{The performance split has a mechanistic explanation.}
        Matrix analysis of $H_\text{res}$ shows that HC learns
        increasingly free, non-doubly-stochastic routing (identity
        distance $0.064 \to 0.951$ across layers), mHC is equally
        aggressive before projection but Sinkhorn-balanced after,
        and mHC-lite is an almost depth-invariant near-uniform mixer.
        The synthetic vs.\ real graph ranking inversion is directly
        visible in the routing statistics.
\end{enumerate}
...
This matches mHC's better real-graph transfer behavior.
